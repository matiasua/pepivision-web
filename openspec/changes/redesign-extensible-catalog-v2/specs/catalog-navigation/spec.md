...
- **WHEN** a visitor requests `/catalogo/coral` (the legacy route) for a product whose default offering is Armazones
- **THEN** the system SHALL issue a permanent redirect to `/catalogo/armazones/coral`
...
- **WHEN** an offering card is rendered for the Lentes de sol ópticos category
- **THEN** its call-to-action SHALL read "Configurar lentes de sol ópticos" rather than a generic or armazón-specific label
...
- **WHEN** "Coral" is offered as both Armazones and Lentes ópticos
- **THEN** the Armazones offering detail page SHALL show a link to the Lentes ópticos offering for the same product, and vice versa
