...
- **Lista de validación manual — primera ejecución (Fase 8, tarea 8.7)**: se revisó el código fuente de las páginas públicas principales (header/menú móvil, inicio, carrusel de marcas, catálogo y filtros, galería/lightbox, cotizador con su paso de adjunto de receta, atención a domicilio, derechos ARCO, FAQ, footer, botón flotante de WhatsApp) y del panel administrativo (login, navegación, formularios de modelo/marca/color/galería, modales de confirmación, solicitudes, usuarios, configuración) contra los seis puntos de la lista anterior. Defectos reales encontrados y corregidos en esta misma fase: `<label>`s sin asociar mediante `htmlFor`/`id` en prácticamente todos los formularios (cotizador, domicilio, ARCO, login, modelo, configuración, usuarios, comunas, derechos ARCO); dos campos administrativos sin ningún `<label>` (solo placeholder); botones de selección del cotizador sin `aria-pressed` y un ✓ decorativo del paso de tratamientos que quedaba en el DOM y se leía aunque no estuviera seleccionado; el drawer de filtros móvil del catálogo funcionaba como modal sin `role="dialog"`/`aria-modal`/trampa de foco/`Escape`; estado activo de navegación y filtros indicado solo por color, sin `aria-current`; mensajes de error/confirmación sin `aria-live` en varios formularios; el botón "Eliminar → ¿Sí/No?" perdía el foco al mostrar/ocultar la confirmación; el acordeón de FAQ ocultaba el contenido colapsado solo visualmente (ahora usa `inert` además de `aria-hidden`, sin perder la transición CSS existente); el pulso infinito del botón de WhatsApp no respetaba `prefers-reduced-motion`. Quedan documentados como defectos menores, no corregidos en esta fase por requerir un patrón de navegación por teclado más elaborado (flechas/`Escape`/cierre por clic externo) que el resto de la implementación no usa todavía: el selector de marca (`BrandSelect`) y el menú de acciones por fotografía en la galería administrativa (`ProductGalleryManager`).
...
- **Rate limiting (implementado en la Fase 8, tarea 8.1)**: además del límite ya existente en el login (Fase 6), los tres formularios públicos (cotizador, domicilio, ARCO) tienen su propio límite por IP y por tipo de formulario (`lib/public-form-rate-limit.ts`), con contadores independientes entre sí (un ataque contra el cotizador no consume ni comparte cupo con el de ARCO). El login además de su límite original por `${ip}:${identifier}` (Fase 6) suma un segundo límite, más laxo, exclusivamente por IP (`modules/auth/service.ts`) — el primero por sí solo permite que un atacante desde una única IP pruebe contra muchos `identifier` distintos sin nunca acumular intentos en el mismo contador (cada combinación IP+identifier es un balde nuevo); el segundo cierra ese hueco limitando el total de intentos fallidos por IP, independientemente de cuántas cuentas distintas se prueben. Toda vez que un límite se activa se registra un evento estructurado (`admin.login_rate_limited` / `public_form.rate_limited`) con la IP y, en el caso de los formularios, el tipo de formulario — nunca con datos de negocio (nombre, correo, mensaje). **Limitación explícita de esta implementación**: los contadores viven en memoria del propio proceso (`Map` en `lib/rate-limit.ts`), por lo que se reinician al reiniciar el contenedor `web` y no se comparten entre réplicas — aceptable para una única instancia (el alcance de esta propuesta), pero un despliegue productivo con más de una réplica de `web` requeriría un backend compartido (Redis u otro almacén centralizado) para que el límite sea efectivo a través de todas las instancias; no se introduce Redis en esta etapa por no ser necesario en un entorno de una sola instancia.
...
- **Cabeceras de seguridad (implementadas en la Fase 8, tarea 8.2)**: una única fuente — `next.config.ts` (`headers()`) — para evitar dos configuraciones (Nginx y Next.js) que puedan contradecirse; `nginx/dev.conf` deliberadamente no agrega ninguna cabecera de seguridad, solo cabeceras de proxy. Se configuran `Content-Security-Policy`, `X-Content-Type-Options: nosniff`, `X-Frame-Options: DENY` (compatibilidad con navegadores pre-CSP3, redundante a propósito con `frame-ancestors 'none'` de la CSP), `Referrer-Policy: strict-origin-when-cross-origin`, `Permissions-Policy` (deniega cámara/micrófono/geolocalización/pago/USB), `Cross-Origin-Opener-Policy: same-origin` y `Cross-Origin-Resource-Policy: same-origin`. La CSP distingue desarrollo de producción vía `NODE_ENV`: `script-src` permite `'unsafe-eval'`/`'unsafe-inline'` únicamente en desarrollo (requerido por React Refresh/HMR de webpack), nunca en producción, donde `next/script`s de Next.js son siempre externos y `'self'` basta; `style-src` mantiene `'unsafe-inline'` en ambos entornos porque varios componentes usan `style={{...}}` inline para valores dinámicos (swatches de color, layout calculado) — una compensación deliberada y de bajo riesgo frente a la complejidad de un pipeline de nonces/hashes para el modelo de amenaza de esta app; `connect-src`/`img-src` incluyen el origen público del almacenamiento de objetos (para las imágenes `unoptimized` de `next/image`) y, solo en desarrollo, el origen del WebSocket de HMR. **HSTS deliberadamente ausente** de esta configuración: enviar `Strict-Transport-Security` solo tiene efecto (y solo es correcto) sobre una conexión ya HTTPS — este entorno de desarrollo sirve siempre por HTTP en `localhost`, y los navegadores ignoran esa cabecera si no llega sobre HTTPS; se documenta aquí como requisito pendiente para cuando exista un proxy productivo con TLS real (se agregaría en Nginx en esa etapa futura, no en este entorno). Verificado con `curl -sD -` contra `/`, `/admin` y `/api/health`.
- **CSRF**: mitigado por cookies `sameSite=lax` (Fase 6) + la verificación de origen integrada de Next.js para Server Actions (compara `Origin` contra el host reconstruido a partir de `Host`/`X-Forwarded-*`, ver `nginx/dev.conf`) — revisado en la Fase 8 confirmando que ninguna acción administrativa evita esta verificación (no hay ninguna ruta API adicional para mutaciones fuera de Server Actions).
- **Principio de mínimo privilegio (implementado en la Fase 8, tarea 8.4)**: `web` ya no se conecta a PostgreSQL como el superusuario de arranque (`POSTGRES_USER`) — se conecta con un rol propio (`APP_DB_USER`, variable `DATABASE_URL_APP`) con permisos únicamente de `SELECT`/`INSERT`/`UPDATE`/`DELETE` sobre el esquema `public`, sin `CREATE`/`DROP`/`ALTER` y sin ser superusuario. Ese rol se aprovisiona de forma idempotente por un nuevo servicio de un solo uso, `db-init` (`db/init-app-role.sql`), que corre después de `migrate` y usa `ALTER DEFAULT PRIVILEGES` para que las tablas creadas por migraciones futuras también queden accesibles al rol automáticamente, sin tener que volver a otorgar permisos a mano. `migrate` sigue usando el superusuario (`DATABASE_URL`) porque necesita DDL para aplicar migraciones de Prisma. Verificado empíricamente: el rol de la app no puede ejecutar `CREATE TABLE` (`permission denied for schema public`) pero sí `SELECT`/lecturas normales de la aplicación.
- **Dependencias (revisado en la Fase 8, tarea 8.5)**: `npm audit` dentro del contenedor `web` encontró 2 vulnerabilidades moderadas, ambas originadas en la copia interna de `postcss@8.4.31` que trae empaquetada `next@16.2.10` (XSS en la serialización de `</style>` sin escapar, corregido en `postcss@8.5.10+`) — no explotable en esta app porque PostCSS solo procesa código CSS propio (Tailwind + hojas de estilo del proyecto) en tiempo de build, nunca CSS proveniente de un usuario. Corregido sin esperar una actualización mayor de Next.js: se agregó `"overrides": { "postcss": "^8.5.19" }` en `package.json`, forzando esa versión también en la copia interna de `next`; verificado que `npm audit` queda en 0 vulnerabilidades y que `npm run build`/`npm test` siguen pasando tras el cambio. Se agrega `.github/dependabot.yml` (actualizaciones semanales del ecosistema npm) para detectar vulnerabilidades futuras de forma continua — `prisma`/`@prisma/client` quedan excluidos solo de actualizaciones mayores automáticas, dado que están fijados a una versión exacta por decisión deliberada del proyecto (Fase 2) y una migración mayor de Prisma requiere la misma revisión manual que cualquier otro cambio arquitectónico. Otras dependencias desactualizadas detectadas por `npm outdated` (p. ej. `@prisma/client`/`prisma` en v6 vs. v7 disponible, `typescript`, `eslint`) no tienen ninguna vulnerabilidad asociada — no se actualizan en esta fase por tratarse de versiones mayores sin una razón de seguridad que lo justifique (regla de "no actualizaciones mayores sin análisis").
- **Auditoría administrativa** (detallada en la sección de autenticación) como control de trazabilidad. **Política ante fallas de auditoría (documentada en la Fase 8)**: `createAuditLogEntry` (`modules/auth/repository.ts`) captura cualquier error al escribir en `AuditLogEntry` (p. ej. una falla transitoria de base de datos) y lo registra por log estructurado en vez de propagarlo — la escritura de auditoría ocurre siempre *después* de que la mutación real ya se aplicó (creación de producto, subida de imagen, cambio de estado de una solicitud, login, etc.), por lo que dejar que esa escritura fallara de forma no capturada podía hacer que una operación ya exitosa se reportara como fallida al administrador (o, en el caso del login, impedir que la sesión se creara pese a que la contraseña era correcta). El trade-off aceptado es una entrada de auditoría ocasionalmente faltante ante una falla real de base de datos, nunca una operación válida reportada como inconsistente.
- **Sin credenciales de AWS en ningún lado (confirmado en la Fase 8, tarea 8.8)**: ni en código, ni en `.env.example`, ni en el propio `.env` de desarrollo, ni en ningún archivo rastreado por Git — verificado con una búsqueda dirigida de patrones de claves de AWS/claves privadas/credenciales sobre todo el árbol versionado; los únicos resultados fueron cadenas de prueba en `tests/notifications-service.test.ts` (fixtures de DKIM, sin material de clave real) y placeholders (`changeme...`) en `.env.example`. `.env` permanece fuera de Git (`.gitignore`).
...
| `DATABASE_URL` (o `POSTGRES_DB`/`POSTGRES_USER`/`POSTGRES_PASSWORD` + host `postgres`) | Conexión del superusuario de arranque de Postgres — usada solo por `migrate` (DDL) y por `db-init` para aprovisionar el rol de aplicación. Ya no la usa `web`. |
| `APP_DB_USER`, `APP_DB_PASSWORD`, `DATABASE_URL_APP` (Fase 8, tarea 8.4) | Rol de aplicación de mínimo privilegio (solo DML), aprovisionado por `db-init` (`db/init-app-role.sql`) — es la conexión real que usa `web` en tiempo de ejecución. |
...
32. Los tres formularios públicos (cotizador, domicilio, ARCO) tienen su propio límite de envíos por IP, independiente entre sí y del límite de login; el login además del límite existente por identificador suma un límite por IP que no puede evadirse probando muchos identificadores distintos desde la misma IP.
33. `next.config.ts` es la única fuente de cabeceras de seguridad de la aplicación (CSP, `X-Content-Type-Options`, `X-Frame-Options`, `Referrer-Policy`, `Permissions-Policy`, `Cross-Origin-Opener-Policy`, `Cross-Origin-Resource-Policy`); la CSP no incluye `unsafe-eval` en producción y no contiene orígenes comodín.
34. `web` se conecta a PostgreSQL con un rol de base de datos que solo tiene permisos de lectura/escritura de datos (sin DDL, sin privilegios de superusuario); únicamente `migrate` usa el superusuario de arranque para aplicar migraciones.
35. `npm audit` no reporta vulnerabilidades conocidas sobre las dependencias del proyecto.
36. Una falla al escribir una entrada de `AuditLogEntry` nunca hace que una mutación administrativa ya exitosa se reporte como fallida al administrador.
37. La lista de validación manual de accesibilidad de la sección "Estrategia de accesibilidad" fue ejecutada al menos una vez sobre las páginas públicas principales y las pantallas clave del panel admin, y los defectos reales encontrados quedaron corregidos o explícitamente documentados como diferidos.
...
| Rate limiting en memoria (sin Redis) se resetea al reiniciar la app y no se comparte entre réplicas | Aceptable para un entorno de desarrollo/instancia única (todos los límites — login e ítems de la Fase 8 en formularios públicos — comparten esta limitación); documentado como upgrade path futuro (backend compartido tipo Redis) si el despliegue productivo llega a correr más de una réplica de `web`. |
...
| El rol de aplicación de mínimo privilegio (`APP_DB_USER`) podría quedar sin los permisos necesarios si una migración futura crea una tabla fuera del flujo normal de `migrate` | `ALTER DEFAULT PRIVILEGES` en `db/init-app-role.sql` cubre automáticamente cualquier tabla/secuencia creada después por el superusuario (`POSTGRES_USER`) en el esquema `public`; el `db-init` también repite el `GRANT` explícito sobre todas las tablas existentes en cada arranque, como red de seguridad idempotente. |
| Una dependencia transitiva empaquetada por un framework (`postcss` dentro de `next`) queda desactualizada sin que el equipo controle directamente esa versión | Corregido con un `overrides` acotado en `package.json` (no una actualización mayor de `next`); revisar si sigue siendo necesario en la próxima actualización mayor de Next.js, que probablemente ya incluya una versión corregida. |
| `BrandSelect` (selector de marca) y el menú de acciones por fotografía en `ProductGalleryManager` no implementan el patrón de teclado completo que sus roles ARIA (`listbox`/`option`, `menu`/`menuitem`) sugieren (flechas, `Escape`, cierre por clic externo) | Documentado como defecto menor y explícitamente diferido en la Fase 8 (ambos son operables con Tab/Enter, solo les falta el patrón de navegación avanzado); revisar si se aborda en una iteración futura de UI o junto con la Fase 9. |
